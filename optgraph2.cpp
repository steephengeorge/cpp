...
public:
  ...
  /* void FindBlocksWithAttribute( std::vector<Block*>& blocksWithAttribute, Block* b)
    {
    ...
    std::vector<Block*> GetBlocksWithAttribute(const std::string& attribute)
    {
        ...
        //std::for_each(m_blocks.begin(), m_blocks.end(), [&blocksWithAttribute] (Block*b){ FindBlocksWithAttribute(blocksWithAttribute, b); } ) need to do this also. or std::copy_if
